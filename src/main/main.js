...
ipcMain.handle('contact-requests:send', async (event, recipientPublicKey, message) => {
  try {
    if (!messaging) {
      return { success: false, error: 'Messaging service not initialized' };
    }

    const request = await messaging.sendContactRequest(recipientPublicKey, message);
...
ipcMain.handle('contact-requests:get-incoming', async () => {
  try {
    if (!currentIdentity) {
      return { success: false, error: 'No identity loaded' };
    }

    const requests = storage.getIncomingContactRequests(currentIdentity.publicKey);
    return { success: true, requests };
  } catch (error) {
    console.error('Error getting incoming contact requests:', error);
    return { success: false, error: error.message };
  }
});

ipcMain.handle('contact-requests:get-outgoing', async () => {
  try {
    if (!currentIdentity) {
      return { success: false, error: 'No identity loaded' };
    }

    const requests = storage.getOutgoingContactRequests(currentIdentity.publicKey);
    return { success: true, requests };
  } catch (error) {
    console.error('Error getting outgoing contact requests:', error);
    return { success: false, error: error.message };
  }
});

ipcMain.handle('contact-requests:accept', async (event, requestId) => {
  try {
    if (!messaging) {
      return { success: false, error: 'Messaging service not initialized' };
    }

    const contact = await messaging.acceptContactRequest(requestId);
...
ipcMain.handle('contact-requests:decline', async (event, requestId) => {
  try {
    if (!messaging) {
      return { success: false, error: 'Messaging service not initialized' };
    }

    const request = await messaging.declineContactRequest(requestId);
    return { success: true, request };
  } catch (error) {
...
function initializeServices(identity) {
  // Initialize P2P network (will auto-connect to bootstrap relays)
  p2p = getP2PInstance();
  ...
  messaging.subscribeToContactRequests();
  ...
  // Contact request event handlers
  messaging.on('contact:request:received', (request) => {
    if (mainWindow) {
      mainWindow.webContents.send('contact:request:received', request);
    }
  });

  messaging.on('contact:request:sent', (request) => {
    if (mainWindow) {
      mainWindow.webContents.send('contact:request:sent', request);
    }
  });

  messaging.on('contact:request:accepted', ({ request, contact }) => {
    if (mainWindow) {
      mainWindow.webContents.send('contact:request:accepted', { request, contact });
    }
  });

  messaging.on('contact:request:declined', (request) => {
    if (mainWindow) {
      mainWindow.webContents.send('contact:request:declined', request);
    }
  });

  messaging.on('contact:request:response:accepted', ({ request, contact }) => {
    if (mainWindow) {
      mainWindow.webContents.send('contact:request:response:accepted', { request, contact });
    }
  });

  messaging.on('contact:request:response:declined', (request) => {
    if (mainWindow) {
      mainWindow.webContents.send('contact:request:response:declined', request);
    }
  });
