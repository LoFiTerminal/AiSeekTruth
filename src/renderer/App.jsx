  ...
  useEffect(() => {
    ...
    if (window.api) {
      ...
      const unsubscribeContactPresence = window.api.onContactPresence((data) => {
        ...
        const previousStatus = useStore.getState().contacts.find(c => c.publicKey === data.publicKey)?.status;
        updateContactPresence(data);

        // Play sound when contact comes online
        if (previousStatus === 'offline' && data.status === 'online') {
          soundManager.contactOnline();
        } else if (previousStatus === 'online' && data.status === 'offline') {
          soundManager.contactOffline();
        }
      });
